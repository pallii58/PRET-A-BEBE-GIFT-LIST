...
// Log which URL source we're using (without exposing password)
if (url) {
  const urlForLog = url.replace(/:[^:@]+@/, ":****@");
  console.log(`[DB] Using connection from: ${process.env.POSTGRES_URL ? 'POSTGRES_URL' : process.env.POSTGRES_PRISMA_URL ? 'POSTGRES_PRISMA_URL' : process.env.DATABASE_URL ? 'DATABASE_URL' : 'individual vars'}`);
  console.log(`[DB] Connection string (masked): ${urlForLog}`);
}
